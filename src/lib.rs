...
impl Guest for Component {
    fn page(edgee_event: Event, settings: Dict) -> Result<EdgeeRequest, String> {
        send_to_kinesis(edgee_event, settings)
    }
...
fn send_to_kinesis(edgee_event: Event, settings_dict: Dict) -> Result<EdgeeRequest, String> {
    let kinesis_settings =
        kinesis_payload::Settings::new(settings_dict).map_err(|e| e.to_string())?;

    // serialize the entire event into JSON
    let record = serde_json::to_string(&edgee_event).unwrap_or_default();
...
mod tests {
    ...
    fn page_works_fine() {
        ...
        let settings = vec![
            ...
            ("kinesis_stream".to_string(), "stream-name".to_string()),
        ];
        ...
        assert_eq!(
            edgee_request
                .url
                .starts_with("https://kinesis.eu-west-1.amazonaws.com/"),
            true
        );
    ...
    fn page_with_partition_key() {
        ...
        let settings = vec![
            ...
            ("aws_region".to_string(), "eu-west-1".to_string()),
            ("kinesis_stream".to_string(), "stream-name".to_string()),
            ("kinesis_partition".to_string(), "abc123".to_string()),
        ];
        let result = Component::page(event, settings);
